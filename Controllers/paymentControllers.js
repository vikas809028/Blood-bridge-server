...
const create_order = async (req, res) => {
  try {
    ...
    if (hospital.bloodStock < quantity) {
      return res.status(400).json({ success: false, message: "Not enough blood available" });
    }
...
const verify_payment = async (req, res) => {
  try {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, 
            quantity, hospitalId, bloodGroup, recipientId, amount } = req.body;

    // Validate inputs
    if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature || 
        !quantity || !hospitalId || !bloodGroup || !recipientId || !amount) {
...
const verify_payment_hospital = async (req, res) => {
  ...
  try {
    ...
    console.log(`Organization validated: ${organization.organisationName}`);

    // 3. Create hospital inventory record (in - receiving from organization)
    console.log('Creating hospital inventory record...');
    ...
    console.log('Hospital inventory record created:', hospitalInventoryRecord._id);
    ...
    const paymentRecord = await Payment.create({
      razorpay_order_id,
      razorpay_payment_id,
      amount: quantity * 100, // Assuming ₹1 per ml (100 paise)
      bloodQuantity: quantity,
      bloodGroup,
    ...
    return res.status(200).json({
      ...
      data: {
        hospitalInventory: hospitalInventoryRecord,
...
// ✅ Export Functions
module.exports = { getRazorPay_keyID, create_order, verify_payment ,verify_payment_hospital};
